 ...
 *   const creds = xsenv.getServices({ uaa: { tag: 'xsuaa' } }).uaa;
...
const JWT_AUTH_ENABLED = String(process.env.JWT_AUTH_ENABLED || '').toLowerCase() === 'true';

const JWT_ALG = process.env.JWT_ALG || 'RS256'; // 'RS256' | 'HS256'
...
const CLOCK_SKEW_SEC = 60; // tolerance for exp / nbf
...
function extractToken(req) {
  ...
  try {
    ...
    const qp = url.searchParams.get('access_token') || url.searchParams.get('token');
